...
const LayoutContent: React.FC = () => {
    ...
    const trimmedSegments =
        pathSegments.length > 1 && pathSegments[0] === "pharmacy"
            ? pathSegments.slice(1)
            : pathSegments;

    const breadcrumbItems = trimmedSegments.map((segment, index) => {
        const path = "/" + ["pharmacy", ...trimmedSegments.slice(0, index + 1)].join("/");
        const label =
            labelMap[segment] ||
    ...
    return (
        <div className={`pharmacy-layout-content ${isCollapsed ? "sidebar-collapsed" : "sidebar-expanded"}`}>
            <header className="pharmacy-layout-header">
            ...
            <main className="pharmacy-layout-main">
                <Outlet />
            </main>
        </div>
    );
