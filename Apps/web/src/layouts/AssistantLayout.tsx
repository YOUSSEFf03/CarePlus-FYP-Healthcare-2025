...
const LayoutContent: React.FC = () => {
    ...
    const trimmedSegments =
        pathSegments.length > 1 && pathSegments[0] === "assistant"
            ? pathSegments.slice(1)
            : pathSegments;

    const breadcrumbItems = trimmedSegments.map((segment, index) => {
        const path = "/" + ["assistant", ...trimmedSegments.slice(0, index + 1)].join("/");
        const label =
            labelMap[segment] ||
    ...
    return (
        <div className={`assistant-layout-content ${isCollapsed ? "sidebar-collapsed" : "sidebar-expanded"}`}>
            <header className="assistant-layout-header">
            ...
            <main className="assistant-layout-main">
                <Outlet />
            </main>
        </div>
    );
    ...
    return (
        <SidebarProvider>
            <div className="assistant-layout">
                <Sidebar onLogout={handleLogout} items={sidebarConfig.assistant}
                    generalItems={getGeneralItems('assistant')} />
                <LayoutContent />
            </div>
        </SidebarProvider>
    );
